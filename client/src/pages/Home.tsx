...
function FeatureView({ active, state, onAction }: { active: string; state: LoveLoopState; onAction: (label: string) => void }) {
  ...
  const uploadMemory = trpc.media.uploadMemory.useMutation({ onSuccess: () => onAction("Memory uploaded") });
  const uploadFile = (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { const dataBase64 = String(reader.result).split(",")[1] || ""; uploadMemory.mutate({ title: file.name, type: file.type.startsWith("video") ? "video" : "photo", fileName: file.name, contentType: file.type || "application/octet-stream", dataBase64 }); }; reader.readAsDataURL(file); };
...
export default function Home() {
  ...
  useEffect(() => { if (!snapshotQuery.data) return; setState(current => ({ ...current, name: snapshotQuery.data.couple.partnerAName || current.name, partner: snapshotQuery.data.couple.partnerBName || current.partner, together: snapshotQuery.data.couple.relationshipDuration || current.together, style: snapshotQuery.data.couple.relationshipStyle || current.style, streak: snapshotQuery.data.couple.streak, xp: snapshotQuery.data.couple.xp, completed: Boolean(snapshotQuery.data.challenge?.completedAt), memories: snapshotQuery.data.memories.map(memory => ({ title: memory.title, type: memory.type, date: new Date(memory.createdAt).toLocaleDateString() })) })); setSentMessages(snapshotQuery.data.messages.slice().reverse().map(messageItem => messageItem.body)); }, [snapshotQuery.data]);
